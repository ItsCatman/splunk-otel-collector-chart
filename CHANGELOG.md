...
- `log`: Update `sending_queue` batch settings when `noDropLogsPipeline` is enabled ([#2076](https://github.com/signalfx/splunk-otel-collector-chart/pull/2076))
...
- `instrumentation`: Reduce Java and .Net auto-instrumentation resource attributes ([#2006](https://github.com/signalfx/splunk-otel-collector-chart/pull/2006))
  This change limits the resource attributes added to the telemetry data emitted by Java and .Net instrumentation libraries to the attributes generated by the `Container` and `Environment` resource providers.
  To enable additional resource providers, update `values.yaml` environment variable `OTEL_JAVA_ENABLED_RESOURCE_PROVIDERS` for Java, or set `OTEL_DOTNET_AUTO_{DETECTOR}_RESOURCE_DETECTOR_ENABLED` for .Net.
  For more information see: https://github.com/signalfx/splunk-otel-collector-chart/blob/main/docs/auto-instrumentation-install.md#4-Resource-Attributes
...
- `clusterReceiver`: Enable k8s.container.status.last_terminated_reason resource attribute for container metrics ([#1968](https://github.com/signalfx/splunk-otel-collector-chart/pull/1968))
...
- `Target Allocator`: Update version to `v0.132.0` ([#2019](https://github.com/signalfx/splunk-otel-collector-chart/pull/2019))
...
In some cases, multiline logs with more than 1000 lines were split into multiple events.
 ([#2012](https://github.com/signalfx/splunk-otel-collector-chart/pull/2012))
